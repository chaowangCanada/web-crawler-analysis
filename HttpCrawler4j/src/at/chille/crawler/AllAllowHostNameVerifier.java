...
/**
 * this class is mostly copied from org.apache.http.conn.ssl.AbstractVerifier.
 * This was nessasary due to final methods that needed to be overwritten
 * 
 * @author chille
 * 
 */
public class AllAllowHostNameVerifier implements X509HostnameVerifier {
	// http://hc.apache.org/httpcomponents-client-ga/httpclient/apidocs/org/apache/http/conn/ssl/AbstractVerifier.html
	...
	public final void verify(final String host, final String[] cns,
		...
		/*
		 * System.out.println("*************"+ host+ "**********"); for(String
		 * cn : cns) { System.out.println("CN: "+cn.toString()); }
		 * System.out.println("*************"+ host+ "**********");
		 */
		// System.out.println("ASDF3: " + host); // used, but no cert available
		// used
	...
	public final boolean verify(String host, SSLSession session) {
		try {
			...
			// System.out.println("ASDF2: " + host + " -> " + certs.length); //
			// not used
			verify(host, x509);
			return true;
		} catch (SSLException e) {
			System.err.println(":-( Invalid Certificate by SSLException in verify(String,SSLSession).");
			return false;
		}
	}
	...
	public final void verify(String host, SSLSocket ssl) throws IOException {
		...
		Certificate[] certs = session.getPeerCertificates();
		// System.out.println("ASDF1: " + host + " -> " + certs.length); // used
		// :-)
		CertificateLogger.getInstance().addCertificateChain(host,
				(X509Certificate[]) certs, ssl, session);
		X509Certificate x509 = (X509Certificate) certs[0];
		verify(host, x509);
	}

	// the following methods are all copied from
	...
	public AllAllowHostNameVerifier() {
		super();
	}
	...
	public final void verify(final String host, final String[] cns,
		...
		for (Iterator<String> it = names.iterator(); it.hasNext();) {
			...
			boolean doWildcard = parts.length >= 3 && parts[0].endsWith("*")
					&& acceptableCountryWildcard(cn) && !isIPAddress(host);

			if (doWildcard) {
				String firstpart = parts[0];
				if (firstpart.length() > 1) { // e.g. server*
					String prefix = firstpart.substring(0,
							firstpart.length() - 1); // e.g. server
					// skip  wildcard part from cn and host
		...
		if (!match) {
			//System.out.println("  (Invalid Certificate: Hostname in Certificate didn't match: "+host+")");
	...
	public static boolean acceptableCountryWildcard(String cn) {
		...
		if (parts.length != 3 || parts[2].length() != 2) {
			return true; // it's not an attempt to wildcard a 2TLD within a
							// country code
		}
	 ...
	 */
	private static String[] getSubjectAlts(final X509Certificate cert,
